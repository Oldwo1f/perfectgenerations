...
exports.addGoogleFontsAndStyles = addGoogleFontsAndStyles;
const fs = require("fs");
const path = require("path");
function addGoogleFontsAndStyles(html, googleFontsLinks) {
    const showIconRandomlyScript = 'function showIconRandomly(icons, containerSelector, numberToShow) {' +
    ...
    try {
        ...
        phosphorCss = phosphorCss.replace(/url\((['"]?)\.\/Phosphor-Duotone\.woff2\1\)/g, 'url("http://localhost:3001/assets/icons/Phosphor-Duotone.woff2")');
        fontawesomeCss = fs.readFileSync(path.join(assetsPath, 'fontawesome.css'), 'utf8');
        fontawesomeCss = fontawesomeCss.replace(/url\((['"]?)\.\/webfonts\//g, 'url("http://localhost:3001/assets/icons/webfonts/');
    }
    catch (error) {
    ...
    if (phosphorCss || fontawesomeCss) {
        const cssInjection = (phosphorCss ? '<style>' + phosphorCss + '</style>' : '') +
