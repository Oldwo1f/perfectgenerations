...
interface TemplateToAdd {
  ...
  isActive: boolean;
  html: string;
  variables: Record<string, { value: string; type: 'text' | 'textarea' }>;
}
...
const templatesToAdd: TemplateToAdd[] = [
  {
    ...
    description:
      "A sleek testimonial card template displaying a user's review, profile photo, name, and star rating. Perfect for showcasing customer feedback on service or hospitality websites. Colors, fonts, and branding are fully customizable.",
    ...
    variables: {
      ...
      reviewText: {
        value:
          'A fantastic stay for the price! The home was tidy, well-furnished, and had all the amenities we needed. The check-in process was seamless and the host was very accommodating.',
        type: 'textarea',
      },
  ...
  },
  {
    ...
    description:
      'A vibrant promotional banner template with customizable text, background colors, and call-to-action elements. Perfect for marketing campaigns and special offers.',
    category: 'Promotions',
  ...
  try {
    ...
    for (const templateData of templatesToAdd) {
      try {
        ...
        // Vérifier si le template existe déjà et le supprimer
        try {
        ...
        // Les variables sont déjà au bon format, pas besoin de conversion
...
// Fonction pour lister tous les templates
async function listAllTemplates(): Promise<void> {
  const app = await NestFactory.createApplicationContext(AppModule);
  const templateService = app.get(TemplateService);
